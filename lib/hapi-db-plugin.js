...
function auth_implementation(server, options) {
  ...
  return {
    authenticate: async (request, reply) => {
      ...
      if (!authorization)
        throw Boom.unauthorized(null, "session_store")
      ...
      // Check the format.
      if (parts.length !== 2) {
        throw  Boom.badRequest("Bad HTTP authentication header format. Expected 'Bearer {token}'", "Bearer")
      }

      else if (parts[0].toLowerCase() !== "bearer") {
        throw Boom.unauthorized("Bad HTTP authentication header format. Missing `Bearer `", "Bearer")
      }
      ...
      return session_model
        ...
        .then(model => {
          ...
          // Success, continue with the request.
            ...
            return reply.authenticated({credentials: model})
          }
        })
    }
  }
