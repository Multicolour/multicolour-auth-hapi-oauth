...
class Twitter {
  ...
  static get_extra_routes(host, server) {
    ...
    server.route([
      {
        method: "GET",
        path: `/session/twitter/app/{username}`,
        config: {
          auth: false,
          handler: (request, reply) => {
            ...
            this.verify_token(request, config, (err, response, tokens) => {
              ...
              // and make sure they're an app.
                ...
                sessions
                  .findOne({ token: tokens.token })
                  .populate("user")
                  .exec((err, found_session) => {
                    // Check for errors.
                    if (err) {
                      reply[host.request("decorator")](Boom.wrap(err), sessions)
                    }
                    // Make sure it's a consumer (app) making this request.
                    else if (found_session && found_session.user && found_session.user.role !== "consumer") {
                      reply[host.request("decorator")](Boom.unauthorized("You don't have authorisation for this action."), users)
                    }
                    else {
